...
def scan_attendance(
    ...
    # 0. Extraer IP y Validar Red Wi-Fi
    client_ip = admin_routes.get_client_ip(request)
    is_network_validated = False
    
    network_setting = db.query(models.NetworkSetting).first()
    if network_setting:
        allowed_list = [ip.strip() for ip in network_setting.allowed_ips.split(",") if ip.strip()]
        is_ip_ok = is_ip_allowed(client_ip, allowed_list)
        
        if network_setting.wifi_validation_enabled:
            if not is_ip_ok:
                raise HTTPException(
                    status_code=403,
                    detail=f"Red no autorizada ({client_ip}): Debes estar conectado a la red Wi-Fi de la oficina para marcar asistencia."
                )
            is_network_validated = True
        else:
    ...
    new_record = models.AttendanceRecord(
        ...
        network_validated=is_network_validated,
        timestamp=now_utc
    )
    db.add(new_record)
    db.commit()
